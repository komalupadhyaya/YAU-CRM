...
import {
    School, Plus, Trash2, CheckCircle2, Clock, Calendar,
    AlertCircle, Edit2, X, Search, User as UserIcon, ChevronDown,
    Loader2, Users, Mail, Phone, FileText, ChevronRight, ChevronLeft, Info,
...
interface Meeting {
    ...
    title: string;
    category: 'school' | 'hr';
    lead_id?: { _id: string; name: string; telephone?: string } | null;
    lead_ids?: { _id: string; name: string; telephone?: string }[];
    date_time: string;
    ...
    return (
        <div ref={containerRef} className="relative">
            <div 
                ...
                className="min-h-[38px] w-full flex flex-wrap items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border hover:border-primary/50 bg-background cursor-text"
            >
            ...
            {open && inputValue.trim() && filteredMembers.length > 0 && (
                <div className="absolute z-50 mt-1 w-full bg-popover border border-border rounded-xl shadow-xl overflow-hidden max-h-44 overflow-y-auto">
                    {filteredMembers.map((m, idx) => (
                        ...
                        >
                            <span className="w-6 h-6 rounded-full bg-primary/20 text-primary font-bold text-[9px] flex items-center justify-center shrink-0">
...
function AvailabilityCalendarModal({
    ...
    // Mode: 'availability' = grid-based slot picker, 'manual' = direct datetime input
    ...
    const attendeeStatusesForDate = useMemo(() => {
        ...
        return internalAttendees.map(attendee => {
            ...
            return { name: attendee.name, status: `Available (${hoursText})`, color: 'text-primary' };
        });
                            ...
                            <div className="space-y-4">
                                <div className="space-y-1.5">
                                    ...
                                    <input
                                        ...
                                        className="w-full h-10 px-3 text-sm rounded-lg border border-border bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/50 transition-all"
                                    />
                                </div>
                                ...
                                {manualDateTime && (
                                    <div className="bg-primary/5 border border-primary/20 rounded-xl p-3 text-center space-y-0.5">
                                        <p className="text-[10px] font-bold text-primary uppercase tracking-wider">Selected</p>
                    ...
                    <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
                        ...
                        <div className="flex-1 p-5 flex flex-col justify-between border-b md:border-b-0 md:border-r border-border h-full overflow-hidden">
                                    ...
                                    {gridDays.map((day, idx) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                `}
                                            >
                                                <span className={`text-[11px] font-bold ${isSelected ? 'text-primary' : ''}`}>{day.getDate()}</span>
                                                {internalAttendees.length > 0 && isCurrentMonth && (
                            ...
                            <div className="flex gap-4 border-t border-border pt-4 text-[10px] text-muted-foreground">
                                <div className="flex items-center gap-1.5">
                                    <span className="w-2.5 h-2.5 rounded bg-primary/10 border border-primary/20"></span> All Free
                                </div>
                                <div className="flex items-center gap-1.5">
...
function MultiUserPicker({
    teamMembers, selected, onChange, placeholder = 'Select attendees...'
}: {
    teamMembers: TeamMember[];
    selected: TeamMember[];
    ...
    return (
        <div ref={ref} className="relative">
            ...
            >
                {selected.map(m => (
                    <span key={m._id} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-primary/10 text-primary text-xs font-medium border border-primary/20">
                        {m.name}
...
function MultiLeadPicker({ leads, selected, onChange }: {
    leads: Lead[];
    selected: Lead[];
    ...
    const remove = (id: string) => onChange(selected.filter(l => l._id !== id));
    ...
    return (
        <div ref={ref} className="relative">
            ...
            >
                {selected.map(l => (
                    <span key={l._id} className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-primary/10 text-primary text-xs font-medium border border-primary/20">
                        <Building size={9} />
                        {l.name}
                        <button type="button" onClick={(e) => { e.stopPropagation(); remove(l._id); }} className="hover:text-destructive transition-colors ml-0.5">
                            <X size={10} />
                        </button>
                    </span>
                ))}
                    ...
                    <div className="max-h-48 overflow-y-auto">
                        {filtered.length === 0 ? (
                            <div className="px-3 py-4 text-center text-sm text-muted-foreground">
                                {leads.length === selected.length ? 'All leads selected' : 'No leads found'}
                            </div>
                        ) : filtered.map(l => (
                            <button
                                key={l._id}
...
function MeetingForm({ teamMembers, leads, editingMeeting, onSuccess, onCancelEdit }: MeetingFormProps) {
    const [step, setStep] = useState(1);
    const [title, setTitle] = useState(editingMeeting?.title ?? '');
    const [selectedLeads, setSelectedLeads] = useState<Lead[]>(
        editingMeeting?.lead_ids && editingMeeting.lead_ids.length > 0
            ? editingMeeting.lead_ids.map(l => ({ _id: l._id, name: l.name, telephone: l.telephone }))
            : editingMeeting?.lead_id
                ? [{ _id: editingMeeting.lead_id._id, name: editingMeeting.lead_id.name }]
                : []
    );
    ...
    const canNavigateToStep = (targetStep: number) => {
        ...
        if (targetStep === 3 && (!title.trim() || selectedLeads.length === 0)) return false;
        if (targetStep === 4 && (!title.trim() || selectedLeads.length === 0 || internalAttendees.length === 0)) return false;
        if (targetStep === 5 && (!title.trim() || selectedLeads.length === 0 || internalAttendees.length === 0)) return false;
        return true;
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!title.trim() || selectedLeads.length === 0 || !dateTime) {
            toast.error('Title, at least one Lead, and Date/Time are required.');
            return;
        }
        ...
        try {
            const payload = {
                title: title.trim(),
                category: 'school',
                lead_ids: selectedLeads.map(l => l._id),
                lead_id: selectedLeads[0]._id,
                date_time: dateTime,
            ...
            onSuccess(res.data);
            if (!isEditing) {
                setTitle(''); setSelectedLeads([]); setDateTime(''); setDuration(30);
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="text-xs font-bold text-primary uppercase tracking-widest text-center py-1">
            ...
            <div className="min-h-[70px] py-1">
                ...
                {step === 4 && (
                    <div className="space-y-1.5 animate-fadeIn">
                        ...
                        <p className="text-[10px] text-muted-foreground">Enter manually (Enter/Comma) or search team member emails to CC.</p>
                    </div>
                )}
                            ...
                            {dateTime ? (
                                <div className="bg-primary/5 border border-primary/20 rounded-xl p-3 flex items-center justify-between">
                                    <div className="space-y-0.5">
                                        <span className="text-[9px] font-bold text-primary uppercase tracking-wider">Confirmed Slot</span>
                                    ...
                                    <Button 
                                        ...
                                        className="h-7 text-[10px] px-2.5 border-primary/20 text-primary hover:bg-primary/10"
                                    >
                                        Change
                                    </Button>
                                </div>
                                ...
                                <div className="bg-rose-500/5 border border-rose-500/10 rounded-xl p-4 text-center space-y-2">
                                    ...
                                    <Button 
                                        type="button" 
                                        onClick={() => setCalendarOpen(true)} 
                                        className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-semibold text-xs py-1.5 h-8 rounded-lg"
                                    >
                                        Pick Date & Time
                                    </Button>
                                </div>
            ...
            <div className="flex gap-2 border-t border-border pt-3 mt-2">
                ...
                {step < 5 ? (
                    <Button type="button" onClick={handleNext} className="flex-1 h-9 text-xs font-semibold">
                        Next
                    </Button>
                ) : (
                    <Button 
                    ...
                    >
                        {isSubmitting ? <><Loader2 size={12} className="animate-spin" /> Saving...</> :
                            isEditing ? <><Edit2 size={12} /> Update Meeting</> : <><Plus size={12} /> Schedule Meeting</>}
                    </Button>
                )}
            </div>
...
function MeetingCard({ meeting, onEdit, onDelete, onViewDetails, canEdit, canDelete }: {
    meeting: Meeting;
    ...
    return (
        <div
            ...
            className={`group relative bg-card border border-l-4 rounded-xl p-3 sm:p-4 transition-all duration-200 cursor-pointer hover:border-primary/40 hover:shadow-sm
                ${meeting.status === 'canceled' ? 'border-l-red-500 opacity-60' :
        ...
        >
            <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                    ...
                    {/* Show lead_ids (multi) or fall back to lead_id */}
                    {((meeting.lead_ids && meeting.lead_ids.length > 0) || meeting.lead_id) && (
                        <div className="flex flex-wrap gap-1 mt-0.5">
                            {(meeting.lead_ids && meeting.lead_ids.length > 0
                                ? meeting.lead_ids
                                : [meeting.lead_id!]
                            ).map(l => (
                                <span key={l._id} className="inline-flex items-center gap-1 text-xs text-muted-foreground">
                                    <School size={9} className="shrink-0 text-primary" />
                    ...
                    {meeting.internal_attendees?.length > 0 && (
                        <div className="flex items-center gap-1 mt-2">
                            <div className="flex -space-x-1.5">
                                {meeting.internal_attendees.slice(0, 5).map(a => (
                                    <AttendeeChip key={a._id} member={a} />
                                ))}
                ...
                <div className="py-4 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="col-span-2 space-y-1">
                            <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Leads / Schools</p>
                            <div className="flex flex-wrap gap-1.5">
                                {(meeting.lead_ids && meeting.lead_ids.length > 0
                                    ? meeting.lead_ids
                                    : meeting.lead_id ? [meeting.lead_id] : []
                                ).map(l => (
                                    <span key={l._id} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-primary/10 text-primary text-xs font-medium border border-primary/20">
                                        <School size={9} /> {l.name}
                                    </span>
                                ))}
                                {!meeting.lead_id && (!meeting.lead_ids || meeting.lead_ids.length === 0) && <span className="text-sm text-muted-foreground">—</span>}
                            </div>
                        </div>
                        <div className="space-y-0.5">
...
export default function SchoolMeetings() {
    ...
    const loadAll = useCallback(async () => {
        try {
            ...
            const [meetingsRes, teamRes, leadsRes] = await Promise.all([
                api.get('/meetings?category=school'),
    ...
    const filtered = meetings.filter(m => {
        const matchesSearch = !searchQuery ||
            ...
            (m.lead_ids && m.lead_ids.some(l => l.name.toLowerCase().includes(searchQuery.toLowerCase()))) ||
            m.lead_id?.name.toLowerCase().includes(searchQuery.toLowerCase());
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 border-b pb-5">
                    <div>
                        ...
                        <p className="text-muted-foreground mt-1 text-sm">
                            Manage and track school outreach meetings.
                        </p>
                    </div>
                    {/* Stats */}
                    <div className="flex items-center gap-2 sm:gap-3 sm:pt-1 shrink-0">
                        <span className="flex items-center gap-1.5 text-xs font-semibold px-2.5 sm:px-3 py-1.5 rounded-full bg-primary/10 text-primary border border-primary/20">
                            <Clock size={11} />
                    ...
                    {permissions.scheduleMeetings && (
                        <div className="lg:sticky lg:top-6">
                            <div className="bg-card border rounded-2xl shadow-sm">
                                <div className={`px-4 sm:px-5 py-4 border-b ${editingMeeting ? "bg-primary/5" : ""}`}>
                        ...
                        <div className="space-y-2.5">
                            {loading ? (
                                <div className="flex items-center justify-center py-20 text-muted-foreground gap-2">
                                    <Loader2 size={18} className="animate-spin text-primary" /> Loading meetings...
                                </div>
                            ) : filtered.length === 0 ? (
                                <div className="p-12 text-center border-2 border-dashed rounded-2xl bg-muted/20 text-muted-foreground text-sm">
                                    ...
                                    <p className="font-semibold text-foreground">No school meetings</p>
                                    <p className="text-sm text-muted-foreground mt-1">
                                        {searchQuery ? "No matching meetings found." : "Schedule your first meeting using the form."}
                                    </p>
                                </div>
                            ) : (
                                filtered.map(m => (
            ...
            <AlertDialog open={!!deletingId} onOpenChange={() => setDeletingId(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>This will permanently delete the meeting. This action cannot be undone.</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                        <AlertDialogAction onClick={() => deletingId && handleDelete(deletingId)} className="bg-destructive hover:bg-destructive/90">
                            Delete
                        </AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
