...
/**
 * Send the Weekly AI Performance Briefing email to executives / team.
 *
 * @param {Object} params
 * @param {string} [params.to='play@yausports.com']
 * @param {string} params.weekRange - e.g. "Sep 7 – Sep 14, 2026"
 * @param {string} params.executiveSummaryHtml - Narrative generated by Claude Sonnet 4.6
 * @param {Object} params.stats - Raw aggregated counts
 * @returns {Promise<{ success: boolean, method?: string, error?: string, html: string }>}
 */
export async function sendWeeklyPerformanceReportEmail({
    ...
    const newLeads7d = stats.leads?.newInLast7Days || 0;
    const totalEALeads = stats.eaLeads?.total || 0;
    const hotEALeads = stats.eaLeads?.byScore?.find(s => s.score === 'Hot')?.count || 0;
    const completedFUs = stats.followups?.completedLast7Days || stats.followups?.totalCompleted || 0;
    const pendingFUs = stats.followups?.totalPending || 0;
    const overdueFUs = stats.followups?.overdue || 0;
    ...
    <body style="margin: 0; padding: 0; background-color: #09090b; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #f4f4f5;">
        <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #09090b; padding: 40px 16px;">
            <tr>
                <td align="center">
                    <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 680px; background-color: #18181b; border: 1px solid #27272a; border-radius: 16px; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.5);">
                        
                        <!-- Header Banner -->
                        <tr>
                            <td style="background: linear-gradient(135deg, #1e3a8a 0%, #2563eb 50%, #4f46e5 100%); padding: 36px 32px; text-align: left;">
                                <div style="display: inline-block; background-color: rgba(255,255,255,0.15); border: 1px solid rgba(255,255,255,0.25); border-radius: 9999px; padding: 4px 14px; font-size: 11px; font-weight: 700; letter-spacing: 0.05em; text-transform: uppercase; color: #ffffff; margin-bottom: 12px;">
                                    ⚡ YAU Executive AI Intelligence
                                </div>
                                <h1 style="margin: 0 0 6px 0; font-size: 24px; font-weight: 800; color: #ffffff; letter-spacing: -0.02em;">
                                    Weekly Performance & Operations Briefing
                                </h1>
                                <p style="margin: 0; font-size: 14px; color: rgba(255,255,255,0.85);">
                                    Week of ${weekRange} • Automated Monday Briefing
                                </p>
                            </td>
                        </tr>
                        ...
                        <!-- Quick KPI Grid -->
                        <tr>
                            <td style="padding: 24px 32px 12px 32px; background-color: #18181b;">
                                <table width="100%" cellpadding="0" cellspacing="0">
                                    <tr>
                                        <td width="25%" style="padding: 6px;">
                                            <div style="background-color: #27272a; border: 1px solid #3f3f46; border-radius: 10px; padding: 14px 12px; text-align: center;">
                                                <div style="font-size: 10px; font-weight: 700; color: #a1a1aa; text-transform: uppercase; letter-spacing: 0.05em;">Total Leads</div>
                                                <div style="font-size: 20px; font-weight: 800; color: #ffffff; margin-top: 4px;">${totalLeads}</div>
                                                <div style="font-size: 10px; color: #22c55e; margin-top: 2px;">+${newLeads7d} this wk</div>
                                            </div>
                                        </td>
                                        <td width="25%" style="padding: 6px;">
                                            <div style="background-color: #27272a; border: 1px solid #3f3f46; border-radius: 10px; padding: 14px 12px; text-align: center;">
                                                <div style="font-size: 10px; font-weight: 700; color: #a1a1aa; text-transform: uppercase; letter-spacing: 0.05em;">EA Inquiries</div>
                                                <div style="font-size: 20px; font-weight: 800; color: #ffffff; margin-top: 4px;">${totalEALeads}</div>
                                                <div style="font-size: 10px; color: #f97316; margin-top: 2px;">🔥 ${hotEALeads} Hot</div>
                                            </div>
                                        </td>
                                        <td width="25%" style="padding: 6px;">
                                            <div style="background-color: #27272a; border: 1px solid #3f3f46; border-radius: 10px; padding: 14px 12px; text-align: center;">
                                                <div style="font-size: 10px; font-weight: 700; color: #a1a1aa; text-transform: uppercase; letter-spacing: 0.05em;">Follow-ups Done</div>
                                                <div style="font-size: 20px; font-weight: 800; color: #38bdf8; margin-top: 4px;">${completedFUs}</div>
                                                <div style="font-size: 10px; color: #a1a1aa; margin-top: 2px;">${pendingFUs} pending</div>
                                            </div>
                                        </td>
                                        <td width="25%" style="padding: 6px;">
                                            <div style="background-color: #27272a; border: 1px solid #3f3f46; border-radius: 10px; padding: 14px 12px; text-align: center;">
                                                <div style="font-size: 10px; font-weight: 700; color: #a1a1aa; text-transform: uppercase; letter-spacing: 0.05em;">Overdue Tasks</div>
                                                <div style="font-size: 20px; font-weight: 800; color: ${overdueFUs > 0 ? '#ef4444' : '#22c55e'}; margin-top: 4px;">${overdueFUs}</div>
                                                <div style="font-size: 10px; color: #a1a1aa; margin-top: 2px;">Needs action</div>
                                            </div>
                                        </td>
                                    </tr>
                                </table>
                        ...
                        <!-- Executive AI Summary Content -->
                        <tr>
                            <td style="padding: 12px 32px 28px 32px; line-height: 1.6; font-size: 14px; color: #e4e4e7;">
                                <div style="background-color: #27272a; border: 1px solid #3f3f46; border-radius: 12px; padding: 24px; margin-top: 8px;">
                                    <div style="margin-bottom: 16px;">
                                        <span style="font-size: 16px; font-weight: 800; color: #ffffff;">🤖 Executive Summary & Strategic Guidance</span>
                                    </div>
                                    <div style="color: #d4d4d8; font-size: 14px;">
                                        ${executiveSummaryHtml}
                                    </div>
                                </div>
                            </td>
                        ...
                        <tr>
                            <td style="padding: 0 32px 32px 32px; text-align: center;">
                                <a href="${crmUrl}" target="_blank" rel="noopener noreferrer" style="display: inline-block; background-color: #2563eb; color: #ffffff; padding: 12px 32px; border-radius: 8px; font-weight: 700; font-size: 14px; text-decoration: none; box-shadow: 0 4px 12px rgba(37,99,235,0.4);">
                        ...
                        <!-- Footer -->
                        <tr>
                            <td style="background-color: #121214; padding: 20px 32px; text-align: center; border-top: 1px solid #27272a;">
                                <p style="margin: 0; font-size: 12px; color: #71717a;">
                                    Youth Athlete University • Performance Intelligence Engine<br>
                                    Delivered automatically every Monday at 8:00 AM EST to <span style="color: #a1a1aa;">${to}</span>
                                </p>
                            </td>
                        </tr>
    ...
    const subject = `📊 YAU Weekly AI Performance Briefing | ${weekRange}`;

    // Attempt Gmail send first
    try {
        await sendMail({
            to,
            subject,
        ...
        console.log(`✅ Weekly AI report sent via Gmail to ${to}`);
        return { success: true, method: 'gmail', html };
    } catch (gmailErr) {
        console.warn(`[Mailer] Gmail send for weekly report failed: ${gmailErr.message}. Attempting SendGrid fallback...`);
        try {
            const { sendSendGridMail } = await import('./sendgrid.service.js');
            await sendSendGridMail({
                to,
                subject,
                html
            });
            console.log(`✅ Weekly AI report sent via SendGrid fallback to ${to}`);
            return { success: true, method: 'sendgrid', html };
        } catch (sgErr) {
            console.error(`❌ Both Gmail and SendGrid failed for weekly report:`, sgErr.message);
            return { success: false, error: sgErr.message, html };
        }
    }
}
